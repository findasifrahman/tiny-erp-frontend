...
export class ListAssetsComponent implements OnInit{
  ...
  displayedColumns: string[] = ['assetentryid','assetname','assetvalue','purchasedate','buttons'];
  displayedColumnsName: string[] = ['assetentryid','assetname','assetvalue','purchasedate','buttons'];
  AllElement: MatTableDataSource<any>;
  loading = false;
  spinner_value = 50;
  maincompanyid = localStorage.getItem('maincompanyid')
  constructor(private snackBar: MatSnackBar, private service: AssetsService, public dialog: MatDialog,
  ...
  onUpdate(id: any) {
    this._router.navigate(['/editAssets', id]);
  }
}
