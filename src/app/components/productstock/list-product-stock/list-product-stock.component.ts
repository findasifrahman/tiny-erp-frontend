...
@Component({
  selector: 'app-list-product-stock',
  templateUrl: './list-product-stock.component.html',
  styleUrl: './list-product-stock.component.scss'
})
export class ListProductStockComponent implements OnInit {
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  displayedColumns: string[] = ['productstockid','productcategoryname','productsubcategoryname','quantity','unit','entrydate','status','buttons'];
  displayedColumnsName: string[] = ['productstockid','productcategoryname','productsubcategoryname','quantity','unit','entrydate','status','buttons'];
  AllElement: MatTableDataSource<any>;
  loading = false;
  spinner_value = 50;
  maincompanyid = localStorage.getItem('maincompanyid')
  constructor(private snackBar: MatSnackBar, private service: ProductStockService, public dialog: MatDialog,
  ...
  onUpdate(id: any) {
    this._router.navigate(['/editStock', id]);
  }
}
