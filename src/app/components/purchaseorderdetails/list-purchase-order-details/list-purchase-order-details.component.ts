...
@Component({
  ...
  styleUrl: './list-purchase-order-details.component.scss'
})
export class ListPurchaseOrderDetailsComponent implements OnInit {
  @ViewChild(MatPaginator, { static: false }) paginator: MatPaginator;
  displayedColumns: string[] = ['purchasedetailid','categoryname','subcategoryname','purchaseamount','purchasequantity','totalamount','buttons'];
  displayedColumnsName: string[] = ['purchasedetailid','categoryname','subcategoryname','purchaseamount','purchasequantity','totalamount','buttons'];
  AllElement: MatTableDataSource<any>;
  loading = false;
  spinner_value = 50;
  constructor(private snackBar: MatSnackBar, private service: PurchaseOrderDetailService, public dialog: MatDialog,
  ...
  onUpdate(id: any) {
    this._router.navigate(['/editPurchaseOrderDetails', id]);
  }
}
