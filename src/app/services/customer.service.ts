...
export class CustomerService {
    ...
    Add(formval : any): Observable<any>{
      ...
      getAll(mid:any): Observable<any> {
        return this.http.get<any>(routeurls.BASE_API_URL + routeurls.CUSTOMER_URL + "/" + mid ).pipe(
          retry(3),
          map(res => {
            console.log(res);
            if (!res) {
      ...
      getbyid(id: number): Observable<any> {
        return this.http.get<any>(routeurls.BASE_API_URL + routeurls.CUSTOMER_URL + "-getbyid", { params: new HttpParams().set('id', id) })
        .pipe(
          retry(3),
          map(res => {
            console.log(res);
      ...
      delete(id: any, mid: any): Observable<any> {
        ...
        let headers = new HttpHeaders({
          'Content-Type': 'application/json'});
        return this.http.delete<any>(routeurls.BASE_API_URL + routeurls.CUSTOMER_URL ,{ params: new HttpParams().set('id', parseInt(id)).set('maincompanyid', mid) });
      }
}